...
这是我在 B 站上偶尔刷到的问题，感觉还是挺有趣的：实现一个 `useWillMount` 的 Hook 来替代 `componentWillMount` 的功能

`componentWillMount` 是类组件的一个生命周期函数，它在组件即将挂载的时候被执行。
...
很明显，我们需要一个开关来控制是否已经挂载，但又不可以用 state 因为 state 会让组件重渲染，如此一来答案只有使用 ref:
